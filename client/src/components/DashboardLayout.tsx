...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!user) return <main className="archive-page flex min-h-screen items-center justify-center p-6" dir="rtl"><section className="w-full max-w-lg border-y archive-rule py-12 text-center"><span className="mx-auto grid size-12 place-items-center rounded-full bg-primary text-primary-foreground"><BookMarked className="size-5" /></span><p className="archive-kicker mt-6">{productIdentity.romanName} · {productIdentity.descriptor}</p><h1 className="mt-3 text-4xl font-semibold tracking-tight">هر روایت، جای خودش را دارد.</h1><p className="mx-auto mt-4 max-w-md leading-8 text-muted-foreground">شخصیت بسازید، مسیر داستان را با انتخاب‌ها تغییر دهید و گفتگو را میان وب و تلگرام ادامه دهید.</p><span className="archive-stamp mt-6"><Sparkles className="size-3" />پرونده‌های زنده، پایان‌های ماندگار</span><p className="archive-number mt-4">{productIdentity.release}</p><Button onClick={startLogin} className="mt-8 rounded-full px-7">ورود به آرشیو شخصی</Button></section></main>;
