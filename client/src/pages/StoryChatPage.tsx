...
export default function StoryChatPage() {
  ...
  if (detail.isLoading) return <div className="archive-page p-10 text-muted-foreground">در حال گشودن پروندهٔ داستان…</div>;
  if (!detail.data) return <div className="archive-page p-10"><Button variant="outline" className="rounded-full border-border bg-transparent" onClick={() => setLocation("/")}>بازگشت به آرشیو</Button></div>;
  const { run, storyBot } = detail.data;
  const ended = run.status === "ended";
  return <div className="archive-page p-3 sm:p-6"><div className="mx-auto grid max-w-[1500px] gap-5 xl:grid-cols-[260px_minmax(0,1fr)_270px]">
    <aside className="hidden border-l archive-rule pl-6 xl:block"><Button variant="ghost" size="sm" onClick={() => setLocation("/")} className="-mr-2 text-muted-foreground"><ArrowRight className="ml-1 size-4" />بازگشت به آرشیو</Button><div className="mt-8"><p className="archive-kicker">نقش تو</p><h2 className="mt-2 text-xl font-semibold">{run.selectedRole}</h2><p className="mt-4 text-sm leading-7 text-muted-foreground">{run.stateSummary}</p></div><div className="mt-8 border-t archive-rule pt-5"><p className="archive-kicker">شرط پایان</p><p className="mt-2 text-sm leading-7">{storyBot.endingConditions}</p></div></aside>
    <section className="story-panel flex min-h-[calc(100vh-7rem)] flex-col overflow-hidden"><header className="flex items-start justify-between gap-4 border-b archive-rule p-6"><div className="flex min-w-0 items-center gap-3"><span className="grid size-11 shrink-0 place-items-center rounded-full bg-secondary text-xl text-secondary-foreground">{storyBot.avatarSymbol}</span><div className="min-w-0"><p className="archive-kicker">{storyBot.visibility === "public" ? "جهان عمومی" : "جهان خصوصی"}</p><h1 className="mt-1 truncate text-xl font-semibold">{run.title}</h1><p className="mt-1 truncate text-xs text-muted-foreground">{storyBot.name}</p></div></div><Badge variant="outline" className="border-border bg-transparent text-muted-foreground">{ended ? "پایان‌یافته" : "در حال روایت"}</Badge></header>
    ...
    <aside className="hidden border-r archive-rule pr-6 xl:block"><div className="flex items-center gap-2 text-[color:var(--moss)]"><Bot className="size-4" /><span className="text-sm font-medium">پروندهٔ شخصیت</span></div><h2 className="mt-4 text-xl font-semibold">{storyBot.name}</h2><p className="mt-3 text-sm leading-7 text-muted-foreground">{storyBot.description}</p><div className="mt-6 flex flex-wrap gap-2">{storyBot.roleOptions.map(role => <span key={role} className="rounded-full bg-secondary px-2.5 py-1 text-xs">{role}</span>)}</div><div className="mt-8 border-t archive-rule pt-5"><p className="archive-kicker">ادامه در تلگرام</p><p className="mt-2 text-xs leading-6 text-muted-foreground">{integrationStatus.data?.isTelegramLinked ? "این اجرا را فعال کنید تا پیام‌های تلگرام وارد همین مسیر شوند." : "ابتدا ربات و حساب تلگرام را از تنظیمات متصل کنید."}</p>{integrationStatus.data?.isTelegramLinked ? <Button variant="outline" disabled={setTelegramActive.isPending} className="mt-3 w-full rounded-full border-border bg-transparent" onClick={() => setTelegramActive.mutate({ storyRunId })}><SendToBack className="ml-2 size-3.5" />فعال‌کردن این داستان</Button> : <Button variant="outline" className="mt-3 w-full rounded-full border-border bg-transparent" onClick={() => setLocation("/settings")}><SendToBack className="ml-2 size-3.5" />اتصال تلگرام</Button>}</div><div className="mt-6 border-t archive-rule pt-5"><p className="archive-kicker">پیگیری داستانی</p><p className="mt-2 text-xs leading-6 text-muted-foreground">با رضایت شما، پس از بی‌فعالیتی یک پیام کوتاه در تلگرام می‌رسد.</p><Input type="number" min={24} max={720} value={followUpHours} onChange={event => setFollowUpHours(Math.max(24, Math.min(720, Number(event.target.value) || 24)))} className="mt-3 h-9 rounded-xl text-xs" aria-label="فاصلهٔ پیگیری به ساعت" /><p className="mt-1 text-[10px] text-muted-foreground">فاصله برحسب ساعت؛ از ۲۴ تا ۷۲۰ ساعت.</p><div className="mt-2 flex flex-wrap gap-2"><Button variant="ghost" disabled={!integrationStatus.data?.isTelegramLinked || updateFollowUp.isPending} className="h-auto px-0 text-xs text-primary hover:bg-transparent hover:text-primary" onClick={() => updateFollowUp.mutate({ storyRunId, isOptedIn: !followUpPreference.data?.isOptedIn, inactivityHours: followUpHours })}>{followUpPreference.data?.isOptedIn ? "خاموش‌کردن پیگیری" : "فعال‌کردن پیگیری"}</Button>{followUpPreference.data?.isOptedIn && <Button variant="ghost" disabled={updateFollowUp.isPending} className="h-auto px-0 text-xs text-primary hover:bg-transparent hover:text-primary" onClick={() => updateFollowUp.mutate({ storyRunId, isOptedIn: true, inactivityHours: followUpHours })}>ذخیرهٔ فاصله</Button>}</div></div><Button variant="ghost" className="mt-5 w-full text-muted-foreground hover:text-destructive" onClick={() => archive.mutate({ storyRunId })}>آرشیو این مسیر</Button></aside>
  </div></div>;
}
